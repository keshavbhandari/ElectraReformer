...
class Reformer(nn.Module):
    def __init__(self, dim, depth, heads=8, dim_head=None, bucket_size=64, n_hashes=8, ff_chunks=100, attn_chunks=None,
                 ...
                 use_rezero=False, use_full_attn=False, full_attn_thres=0, reverse_thres=0, num_mem_kv=0,
...
class ReformerLM(nn.Module):
    def __init__(self, num_tokens, dim, depth, max_seq_len, heads=8, dim_head=64,
                 bucket_size=64, n_hashes=4, ff_chunks=100, attn_chunks=1,
                 causal=False, weight_tie=False, lsh_dropout=0., ff_dropout=0.,
                 ff_mult=4, ff_activation=None, ff_glu=False, post_attn_dropout=0.,
                 layer_dropout=0., random_rotations_per_head=False, use_scale_norm=False,
        ...
        if emb_type == "default":
            ...
            self.token_emb = QREmbeddingBag(num_categories=num_tokens, embedding_dim=emb_dim,
                                            num_collisions=num_collisions)
        elif emb_type == "md":
            self.token_emb = PrEmbeddingBag(num_embeddings=num_tokens, proj_embedding_dim=emb_dim // 2,
                                            base_dim=emb_dim)
        else:
